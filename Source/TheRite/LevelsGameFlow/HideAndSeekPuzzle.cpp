...
void AHideAndSeekPuzzle::OnNextPuzzleStep(AInteractor* Interactable)
{
	if(!bFirstInteractionDone)
	{
		...
		PostProcesModifierClass->ModifyPostProcessValues(PostProcessModiferValue, .35f);
	...
	{
		PostProcesModifierClass->ModifyPostProcessValues(PostProcessModiferValue, 0.75f);
	}
